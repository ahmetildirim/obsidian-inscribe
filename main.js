...
var DEFAULT_TEMPLATE = `{{pre_cursor}} {{active_sentence}} {{post_cursor}}`;
function buildPrompt(template, args) {
  return mustache_default.render(template, args);
}
function preparePrompt(editor, template = DEFAULT_TEMPLATE) {
...
var InscribeSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  async displayOllamaSettings() {
    ...
    new import_obsidian.Setting(containerEl).setName("User Prompt").setDesc("Enter the user prompt.").addTextArea(
      (text) => text.setPlaceholder(settings.user_prompt).setValue(settings.user_prompt).onChange(async (value) => {
        settings.user_prompt = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("System Prompt").setDesc("Enter the system prompt.").addTextArea(
      (text) => text.setPlaceholder(settings.system_prompt).setValue(settings.system_prompt).onChange(async (value) => {
