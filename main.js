...
var InscribeSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  async renderProviderSection() {
    ...
    await this.createProviderSetting("ollama" /* OLLAMA */);
    await this.createProviderSetting("openai" /* OPENAI */);
  }
  async createProviderSetting(type) {
    const providerConfig = type === "ollama" /* OLLAMA */ ? { name: UI_STRINGS.PROVIDERS.OLLAMA.NAME, desc: UI_STRINGS.PROVIDERS.OLLAMA.DESC } : { name: UI_STRINGS.PROVIDERS.OPENAI.NAME, desc: UI_STRINGS.PROVIDERS.OPENAI.DESC };
    const isConfigured = this.plugin.settings.providers[type].configured;
    new import_obsidian2.Setting(this.sections.providers).setName(providerConfig.name).setDesc(providerConfig.desc).addExtraButton((button) => {
      button.setIcon(isConfigured ? "check-circle" : "alert-circle").setTooltip(isConfigured ? UI_STRINGS.TOOLTIPS.CONFIGURED : UI_STRINGS.TOOLTIPS.NOT_CONFIGURED);
    }).addButton((button) => {
      button.setIcon("settings").setTooltip(`${UI_STRINGS.TOOLTIPS.CONFIGURE} ${providerConfig.name}`).onClick(() => this.openProviderModal(type));
    });
  }
  ...
  async createProfileManager() {
    new import_obsidian2.Setting(this.sections.profiles).setHeading().setName(UI_STRINGS.PROFILES.MANAGE.NAME).setDesc(UI_STRINGS.PROFILES.MANAGE.DESC).addDropdown(this.createProfileDropdown.bind(this)).addExtraButton(this.createNewProfileButton.bind(this)).addExtraButton(this.createDeleteProfileButton.bind(this));
  }
  ...
  createNewProfileButton(button) {
    button.setIcon("plus").setTooltip(UI_STRINGS.TOOLTIPS.CREATE_PROFILE).onClick(async () => {
  ...
  createDeleteProfileButton(button) {
    ...
    button.setDisabled(isDefault).setIcon("trash").setTooltip(isDefault ? UI_STRINGS.TOOLTIPS.CANT_DELETE_DEFAULT : UI_STRINGS.TOOLTIPS.DELETE_PROFILE).onClick(async () => {
...
var SettingsGroup = class {
  constructor(container, profile, plugin) {
...
var ProfileBasicSettingsGroup = class extends SettingsGroup {
  async render() {
    new import_obsidian2.Setting(this.container).setName("Profile Name").setDesc("Name of the profile").addText((text) => {
      text.setValue(this.profile.name).onChange(async (value) => {
...
var ProfileModelSettingsGroup = class extends SettingsGroup {
  async render() {
    new import_obsidian2.Setting(this.container).setName("AI Provider").setDesc("Choose your preferred AI provider").addDropdown((dropdown) => {
      dropdown.addOption("ollama" /* OLLAMA */, "Ollama").addOption("openai" /* OPENAI */, "OpenAI").setValue(this.profile.provider).onChange(async (value) => {
        this.profile.provider = value;
        await this.saveSettings();
      });
    });
    new import_obsidian2.Setting(this.container).setName("Model").setDesc("Select the model to use for completions").addDropdown((dropdown) => {
      const models = this.profile.provider === "ollama" /* OLLAMA */ ? this.plugin.settings.providers.ollama.models : this.plugin.settings.providers.openai.models;
      models.forEach((model) => dropdown.addOption(model, model));
      dropdown.setValue(this.profile.completionOptions.model).onChange(async (value) => {
...
var ProfileCompletionSettingsGroup = class extends SettingsGroup {
  async render() {
    new import_obsidian2.Setting(this.container).setName("Temperature").setDesc("Control the randomness of completions (0 = deterministic, 1 = creative)").addSlider((slider) => {
      slider.setLimits(0, 1, 0.1).setValue(this.profile.completionOptions.temperature).setDynamicTooltip().onChange(async (value) => {
        this.profile.completionOptions.temperature = value;
        await this.saveSettings();
      });
    });
    new import_obsidian2.Setting(this.container).setName("Suggestion Delay").setDesc("Delay in milliseconds before fetching suggestions").addSlider((slider) => {
      slider.setLimits(0, 2e3, 100).setValue(this.profile.delayMs).setDynamicTooltip().onChange(async (value) => {
        this.profile.delayMs = value;
        await this.saveSettings();
      });
    });
    new import_obsidian2.Setting(this.container).setName("Completion Strategy").setDesc("Choose how completions should be split and accepted").addDropdown((dropdown) => {
      dropdown.addOption("word", "Word by Word").addOption("sentence", "Sentence by Sentence").addOption("paragraph", "Paragraph by Paragraph").addOption("full", "Full Completion").setValue(this.profile.splitStrategy).onChange(async (value) => {
...
var ProfilePromptSettingsGroup = class extends SettingsGroup {
  async render() {
    new import_obsidian2.Setting(this.container).setName("System Prompt").setDesc("Set the system prompt that defines the AI's behavior").addTextArea((text) => {
      text.inputEl.rows = 3;
      text.inputEl.style.width = "100%";
      text.setValue(this.profile.completionOptions.systemPrompt).onChange(async (value) => {
        this.profile.completionOptions.systemPrompt = value;
        await this.saveSettings();
      });
    });
    new import_obsidian2.Setting(this.container).setName("User Prompt").setDesc("Configure the prompt template for completions").addExtraButton((button) => {
      button.setIcon("list").setTooltip(UI_STRINGS.TOOLTIPS.INSERT_VARIABLES).onClick(async () => {
      ...
      text.inputEl.rows = 3;
      text.inputEl.style.width = "100%";
      text.setValue(this.profile.completionOptions.userPrompt).onChange(async (value) => {
...
function resolveProfile(settings, providers) {
